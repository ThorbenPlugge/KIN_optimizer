...
def kepler_step_internal(kc, dt, beta, b, directionVector, velocityVector, r0, eta, zeta):
    ...
    if not returnBool:
        return directionVector, velocityVector, returnBool, newton_solution
    else:
        fhat = -(ca / r0)
        g = eta * G2 + r0 * G1
        fdot = -bsa
...
def kepler_step_depth_iterative(kc, dtGlobal, beta, b, directionVector, velocityVector, r0, eta, zeta):
    ...
    while depth < 30 and depth != -1:
        ...
        if currentElement == 0:
            ...
            directionVector, velocityVector, did_it_work, newton_solution = kepler_step_internal(
    ...
    return directionVector, velocityVector, np.stack(newton_solutions)
...
def kepler_step(kc, dt, directionVector, velocityVector):
    ...
    r1, v1, newton_solutions = kepler_step_depth_iterative(
        kc, dt, beta, b, directionVector, velocityVector, r0, eta, zeta)

    # return r1, v1, newton_solutions

    padding_needed = 4 - newton_solutions.shape[0]
    if (newton_solutions.shape[0] > 1):
        print("deeper than 1")
    if padding_needed < 0:
        # If the array already has 4 or more rows, truncate it to the first 4 rows
        print("There are more than 4 newton solutions. Please make this number larger")
        return r1, v1, newton_solutions
