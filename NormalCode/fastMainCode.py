import numpy as np
import NormalCode.newton_storing_ks as ks
...
def kepler_solver(tau, mij, r0, v0):
    if mij == 0:
        ...
        r0, v0, newton_solutions = ks.kepler_step(keplerConstant, tau, r0, v0)

        return r0, v0, newton_solutions


def do_step(tau, n, m, r, v):
    tauDiv2 = tau * 0.5
    r = r + v * tauDiv2
    r, v, newton_solutions = evolve_HW(tau, n, m, r, v)
    r = r + v * tauDiv2
    return r, v, newton_solutions
...
def evolve_HW(tau, n, m, r, v):
    ...
    r1_list = []
    v1_list = []
    newton_solutions_list = []
    for tau_i, mij_i, r0_i, vv0_i in zip(tau_flat, mij_flat, r0_flat, vv0_flat):
        r1_i, v1_i, newton_solutions_i = kepler_solver(
            tau_i, mij_i, r0_i, vv0_i)
        r1_list.append(r1_i)
        v1_list.append(v1_i)
        newton_solutions_list.append(newton_solutions_i)

    r1 = np.array(r1_list)
    v1 = np.array(v1_list)
    newton_solutions = np.array(newton_solutions_list)

    newton_solutions = newton_solutions.reshape(n, n, 4)
    r1 = r1.reshape(n, n, 3)
    v1 = v1.reshape(n, n, 3)
